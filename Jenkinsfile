    ...
    stage("Skip Gate") {
      ...
      stages {
        ...
        stage("Prepare Deployment") {
          when { not { changelog '.*\\[skip deploy\\].*' } } //Skip deployment if commit requests it
          parallel {
            stage("Production") {
              when { branch 'master' }
              steps {
        ...
        stage("Deploy") {
          when { 
            allOf {
              expression { env.CHANGE_ID == null }  //Don't deploy a "PR"-Branch
              not { changelog '.*\\[skip deploy\\].*' } //Skip deployment if commit requests it
            }
          }
          steps {
